def post(self):
    ...
    if parsed_info["type"] == "PinPoint":        
        ...
        if "subtract_minute" in parsed_info.keys():
            if parsed_info["subtract_minute"] == "1":
        ...
        assignment_details = QuadrantAssignmentDetails(
            identifier=Helpers.guid(),
            quadrant_identifier=quadrant_entry.identifier,
            dt=Helpers.pacific_now()
        )        
        ...
        if not quadrant is None:
            ...
            quad_details = QuadrantAssignmentDetails.first(QuadrantAssignmentDetails.quadrant_identifier == quadrant.identifier)
            if not quad_details is None:
                quad_details.dt = Helpers.pacific_now()
                quad_details.put()
        ...
        for p in points:
            ...
            for att in atts:
                ...
                #     att = json.loads(attribute)
                item[att] = attribute
            combined = str(p.latitude) + "_" + str(p.longitude)
            item["blacklisted"] = (combined in blacklist_coords)
        ...
        if not quadrant is None:
            ...
            details = QuadrantAssignmentDetails.first(QuadrantAssignmentDetails.quadrant_identifier == parsed_info["identifier"])
        ...
        if not rep is None:
            ...
            #changing to OR TRUE because deactivated users will appear in the daily digest emails
            if stat is None or True:
                stat = LeaderBoardStat(
                    identifier=Helpers.guid(),
    ...
    elif parsed_info["type"] == "ColeQuery":
        #staging below
        #api_url = "https://cib2-dev.waveinteractive.com/contacts/search"
        #prod below
        ###########
        api_url = "https://fetch.coleinformation.com/contacts/search"
        something = "something"
        form_data = {}
        form_data["Address"] = parsed_info["address"]
        form_data["State"] = parsed_info["state"]
        form_data["Zip"] = parsed_info["postal"]

        req_headers = {}
        req_headers["Content-Type"] = "application/json"
        req_headers["Cole-ApiKey"] = "46d65970-e2f2-4d24-9c87-c126ee5e638f"
        req_headers["DuplicateCheck"] = "override"
        ...
        #ret_json = api_response

        ret_json = {"first_names": [], "last_names": [], "homeowner_predictions": [], "possible_interests": [], "scores": [], "dobs": [], "credits": []}

        cnt = 0
        for result in api_response["Results"]:
            if cnt == 3:
                break
            ret_json["first_names"].append(result["FirstName"])
            ret_json["last_names"].append(result["LastName"])
            ret_json["homeowner_predictions"].append(result["HomeownerConfirmed"])
            ret_json["scores"].append(result["Score"])
            ret_json["dobs"].append(result["DateOfBirth"])
            ret_json["credits"].append(result["Financial_CreditRating"])
            ...
            for key in result.keys():
                if "Interest_" in key:
                    ...
                    if len(value) > 0:
                        replaced = key.replace("Interest_", "")
                        split_values = replaced.split("_")
                        interests.append(" ".join(split_values))

            ret_json["possible_interests"].append(interests)

            cnt += 1

        if "debugging" in parsed_info.keys():
            ret_json = api_response["Results"]
